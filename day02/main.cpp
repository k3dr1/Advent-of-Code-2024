...
auto is_safe(auto&& diffs) -> bool {
    return std::ranges::fold_left(diffs, true,
                   [](const auto& l, const auto& r) { return l && (0 < r && r < 4); })
        || std::ranges::fold_left(diffs, true,
            [](const auto& l, const auto& r) { return l && (-4 < r && r < 0); });
}
...
auto is_safe2(auto&& diffs) -> bool
{
    ...
    for (const auto missing : std::views::iota(0z, std::ranges::distance(diffs))) {
        auto r = std::views::enumerate(diffs)
            | std::views::filter([missing](const auto& p) -> bool {
                  const auto& [i, v] = p;
                  return { i != missing };
              })
            | std::views::transform(
...
auto main() -> int {
    ...
    for (auto line : lines) {
        auto nums = line
            | std::views::split(' ')
            | std::ranges::views::transform( [](const auto& s) { return std::string(&*s.begin(), std::ranges::distance(s)); })
